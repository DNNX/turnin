...
prop_noFindAllBadHints rtN rN tN cN gN pN trN suffix =
 "" `notElem` [rtN,rN,tN,cN,gN,pN,trN,suffix] ==>
  let tr = make $ trN ++ suffix
     ...
     [] == find (one rN) rt                        && Nothing == findUnambiguous (one rN) rt                       
  && [] == find (one tN) r                         && Nothing == findUnambiguous (one tN) r                        
  && [] == find (one cN) t                         && Nothing == findUnambiguous (one cN) t                        
  && [] == find (one gN) c                         && Nothing == findUnambiguous (one gN) c                        
  && [] == find (one pN) g                         && Nothing == findUnambiguous (one pN) g                        
  && [] == find (one sRN) p                        && Nothing == findUnambiguous (one sRN) p                       
  && [] == find (one tfRN) p                       && Nothing == findUnambiguous (one tfRN) p                      
  && [] == find (one trRN) p                       && Nothing == findUnambiguous (one trRN) p                      
  && [] == find (one trN) trr                      && Nothing == findUnambiguous (one trN) trr                     
                                                                                             
  && [] == find (two rN tN) rt                     && Nothing == findUnambiguous (two rN tN) rt                    
  && [] == find (two tN cN) r                      && Nothing == findUnambiguous (two tN cN) r                     
  && [] == find (two cN gN) t                      && Nothing == findUnambiguous (two cN gN) t                     
  && [] == find (two gN pN) c                      && Nothing == findUnambiguous (two gN pN) c                     
  && [] == find (two pN sRN) g                     && Nothing == findUnambiguous (two pN sRN) g                    
  && [] == find (two pN tfRN) g                    && Nothing == findUnambiguous (two pN tfRN) g                   
  && [] == find (two pN trRN) g                    && Nothing == findUnambiguous (two pN trRN) g                   
  && [] == find (two pN trRN) p                    && Nothing == findUnambiguous (two pN trRN) p                   
                                                                                             
  && [] == find (three rN tN cN) rt                && Nothing == findUnambiguous (three rN tN cN) rt               
  && [] == find (three tN cN gN) r                 && Nothing == findUnambiguous (three tN cN gN) r                
  && [] == find (three cN gN pN) t                 && Nothing == findUnambiguous (three cN gN pN) t                
  && [] == find (three gN pN sRN) c                && Nothing == findUnambiguous (three gN pN sRN) c               
  && [] == find (three gN pN tfRN) c               && Nothing == findUnambiguous (three gN pN tfRN) c              
  && [] == find (three gN pN trRN) c               && Nothing == findUnambiguous (three gN pN trRN) c              
  && [] == find (three pN trRN trN) g              && Nothing == findUnambiguous (three pN trRN trN) g             
                                                                                             
  && [] == find (four rN tN cN gN) rt              && Nothing == findUnambiguous (four rN tN cN gN) rt             
  && [] == find (four tN cN gN pN) r               && Nothing == findUnambiguous (four tN cN gN pN) r              
  && [] == find (four cN gN pN sRN) t              && Nothing == findUnambiguous (four cN gN pN sRN) t             
  && [] == find (four cN gN pN tfRN) t             && Nothing == findUnambiguous (four cN gN pN tfRN) t            
  && [] == find (four cN gN pN trRN) t             && Nothing == findUnambiguous (four cN gN pN trRN) t            
  && [] == find (four gN pN trRN trN) c            && Nothing == findUnambiguous (four gN pN trRN trN) c           
                                                                                             
  && [] == find (five rN tN cN gN pN) rt           && Nothing == findUnambiguous (five rN tN cN gN pN) rt          
  && [] == find (five tN cN gN pN sRN) r           && Nothing == findUnambiguous (five tN cN gN pN sRN) r          
  && [] == find (five tN cN gN pN tfRN) r          && Nothing == findUnambiguous (five tN cN gN pN tfRN) r         
  && [] == find (five tN cN gN pN trRN) r          && Nothing == findUnambiguous (five tN cN gN pN trRN) r         
  && [] == find (five cN gN pN trRN trN) t         && Nothing == findUnambiguous (five cN gN pN trRN trN) t        
                                                                                             
  && [] == find (six rN tN cN gN pN trRN) rt       && Nothing == findUnambiguous (six rN tN cN gN pN trRN) rt      
  && [] == find (six rN tN cN gN pN tfRN) rt       && Nothing == findUnambiguous (six rN tN cN gN pN tfRN) rt      
  && [] == find (six rN tN cN gN pN sRN) rt        && Nothing == findUnambiguous (six rN tN cN gN pN sRN) rt       
  && [] == find (six rN cN gN pN trRN trN) r       && Nothing == findUnambiguous (six rN cN gN pN trRN trN) r      
                                                                                             
  && [] == find (seven rN tN cN gN pN trRN trN) rt && Nothing == findUnambiguous (seven rN tN cN gN pN trRN trN) rt
...
noFindByName bad rt allPossibilities = all f (filter (contains bad) allPossibilities)
 where f s = [] == find s rt && Nothing == ((==[]).(`find`rt)) s rt
