...
prop_noFindNoHints rootN rN tN cN gN pN trN =
 "" `notElem` [rootN,rN,tN,cN,gN,pN,trN] ==>
  ...
  && [[]] == nub (map (find one') [g0])
--  && [[]] == (nub $ map (find one') [])
  && [[]] == nub (map (find one') [trr0])
  && prs1 == find one' p0

  && [[]] == nub (map (find two') [rt0,rt1])
  && [[]] == nub (map (find two') [r0,r1])
  && [[]] == nub (map (find two') [t0,t1])
  && [[]] == nub (map (find two') [c0,c1])
  && [[]] == nub (map (find two') [g0])
  && [[]] == nub (map (find two') [p0,p1])
  && prs2 == find two' g1

  && [[]] == nub (map (find three') [rt0,rt1,rt2])
  && [[]] == nub (map (find three') [r0,r1,r2])
  && [[]] == nub (map (find three') [t0,t1,t2])
  && [[]] == nub (map (find three') [c0,c1])
  && [[]] == nub (map (find three') [g0,g1,g2])
  && prs3 == find three' c2

  && [[]] == nub (map (find four') [rt0,rt1,rt2,rt3])
  && [[]] == nub (map (find four') [r0,r1,r2,r3])
  && [[]] == nub (map (find four') [t0,t1,t2])
  && [[]] == nub (map (find four') [c0,c1,c2,c3])
  && prs4 == find four' t3

  && [[]] == nub (map (find five') [rt0,rt1,rt2,rt3,rt4])
  && [[]] == nub (map (find five') [r0,r1,r2,r3])
  && [[]] == nub (map (find five') [t0,t1,t2,t3,t4])
  && prs5 == find five' r4

  && [[]] == nub (map (find six') [rt0,rt1,rt2,rt3,rt4])
  && [[]] == nub (map (find six') [r0,r1,r2,r3,r4,r5])
  && prs6 == find six' rt5

  && [[]] == nub (map (find seven') [rt0,rt1,rt2,rt3,rt4,rt5,rt6])
...
prop_noFindAllGoodHints rootN rN tN cN gN pN trN =
 "" `notElem` [rootN,rN,tN,cN,gN,pN,trN] ==>
  ...
  && [[]] == nub (map (find (one tN))  [r0])
  && [[]] == nub (map (find (one cN))  [t0])
  && [[]] == nub (map (find (one gN))  [c0])
  && [[]] == nub (map (find (one pN))  [g0])
  && [[]] == nub (map (find (one trN)) [trr0])

  && [[]] == nub (map (find (two rN tN))    [rt0,rt1])
  && [[]] == nub (map (find (two tN cN))    [r0,r1])
  && [[]] == nub (map (find (two cN gN))    [t0,t1])
  && [[]] == nub (map (find (two gN pN))    [c0,c1])
  && [[]] == nub (map (find (two pN trRN))  [p0,p1])

  && [[]] == nub (map (find (three rN tN cN))    [rt0,rt1,rt2])
  && [[]] == nub (map (find (three tN cN gN))    [r0,r1,r2])
  && [[]] == nub (map (find (three cN gN pN))    [t0,t1,t2])
  && [[]] == nub (map (find (three pN trRN trN)) [g0,g1,g2])

  && [[]] == nub (map (find (four rN tN cN gN))    [rt0,rt1,rt2,rt3])
  && [[]] == nub (map (find (four tN cN gN pN))    [r0,r1,r2,r3])
  && [[]] == nub (map (find (four cN gN pN trN))   [t0,t1,t2,t3])
  && [[]] == nub (map (find (four gN pN trRN trN)) [c0,c1,c2,c3])

  && [[]] == nub (map (find (five rN tN cN gN pN))    [rt0,rt1,rt2,rt3,rt4])
  && [[]] == nub (map (find (five tN cN gN pN trN))   [r0,r1,r2,r3,r4])
  && [[]] == nub (map (find (five cN gN pN trRN trN)) [t0,t1,t2,t3,t4])

  && [[]] == nub (map (find (six rN cN gN pN trRN trN))  [rt0,rt1,rt2,rt3,rt4,rt5])
  && [[]] == nub (map (find (six cN gN pN trRN trN trN)) [r0,r1,r2,r3,r4,r5])

  && [[]] == nub (map (find (seven rN tN cN gN pN trRN trN)) [rt0, rt1, rt2, rt3, rt4, rt5, rt6])
...
prop_noFindAllBadHints rtN rN tN cN gN pN trN suffix =
 "" `notElem` [rtN,rN,tN,cN,gN,pN,trN,suffix] ==>
  ...
  && [] == find (three rN tN cN) rt
  && [] == find (three tN cN gN) r
  && [] == find (three cN gN pN) t
  && [] == find (three gN pN sRN) c
  && [] == find (three gN pN tfRN) c
  && [] == find (three gN pN trRN) c
  && [] == find (three pN trRN trN) g

  && [] == find (four rN tN cN gN) rt
  && [] == find (four tN cN gN pN) r
  && [] == find (four cN gN pN sRN) t
  && [] == find (four cN gN pN tfRN) t
  && [] == find (four cN gN pN trRN) t
  && [] == find (four gN pN trRN trN) c

  && [] == find (five rN tN cN gN pN) rt
  && [] == find (five tN cN gN pN sRN) r
  && [] == find (five tN cN gN pN tfRN) r
  && [] == find (five tN cN gN pN trRN) r
  && [] == find (five cN gN pN trRN trN) t

  && [] == find (six rN tN cN gN pN trRN) rt
  && [] == find (six rN tN cN gN pN tfRN) rt
  && [] == find (six rN tN cN gN pN sRN) rt
  && [] == find (six rN cN gN pN trRN trN) r

  && [] == find (seven rN tN cN gN pN trRN trN) rt
...
noFindByName bad rt allPossibilities = all ((==[]).(`find`rt)) $ filter (contains bad) allPossibilities
...
findNoneFromRoot rt [] = null(find one' rt)
                      && null(find two' rt)
                      && null(find three' rt)
                      && null(find four' rt)
                      && null(find five' rt)
...
findNoneFromRoot rt [r] = null(find (S z$ one r) rt)
                       && null(find (S z$S z$ one r) rt)
                       && null(find (S z$S z$S z$ one r) rt)
                       && null(find (S z$S z$S z$S z $one r) rt)
                       && null(find (S z$S z$S z$S z$S z$ one r) rt)
                       && null(find (S z$S z$S z$S z$S z$S z$ one r) rt)

findNoneFromRoot rt [r,t] = null(find (S z$ two r t) rt)
                         && null(find (S z$S z$ two r t) rt)
                         && null(find (S z$S z$S z $ two r t) rt)
                         && null(find (S z$S z$S z$S z$ two r t) rt)
                         && null(find (S z$S z$S z$S z$S z$ two r t) rt)

findNoneFromRoot rt [r,t,c] = null(find (S z$ three r t c) rt)
                           && null(find (S z$S z $ three r t c) rt)
                           && null(find (S z$S z$S z$ three r t c) rt)
                           && null(find (S z$S z$S z$S z$ three r t c) rt)

findNoneFromRoot rt [r,t,c,g] = null(find (S z $ four r t c g) rt)
                             && null(find (S z$S z$ four r t c g) rt)
                             && null(find (S z$S z$S z$ four r t c g) rt)

findNoneFromRoot rt [r,t,c,g,p] = null(find (S z$ five r t c g p) rt)
                               && null(find (S z$S z$ five r t c g p) rt)

findNoneFromRoot rt [r,t,c,g,p,pr] = null(find (S z$ six r t c g p pr) rt)
